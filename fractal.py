...
def get_fractal_data(mesh, iters = 20, n_thread_per_block_side = 25):
    """
    Galculates fractal data for given parameters:
    :param mesh: Mesh object to describe exact bounds and dimensions of fractal image complex field
    ...
    device_c = cuda.to_device(mesh.mesh)
    device_result = cuda.device_array_like(np.zeros(mesh.total_points).reshape(mesh.dims[0], mesh.dims[1]))

    #Calculate thread grid clusters and their dimeniosn for calculation
    n_grid = (mesh.dims[0]//n_thread_per_block_side, mesh.dims[1]//n_thread_per_block_side)
