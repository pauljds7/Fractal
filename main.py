...
class ProgramGui():
    def __init__(self, mesh, window_name = "Fractal", info_panel_height_px = 100, device_info = None, bg_color = 'grey'):
        self.root = tk.Tk()
        ...
        self.window_size_px = (mesh.dims[0], mesh.dims[1]+info_panel_height_px) #Window size based on iamge size + padding for info panel
        self.root.geometry(str(self.window_size_px[0]) + 'x' + str(self.window_size_px[1])) #
        self.img_size_inch = (math.ceil(mesh.dims[0] * self.inch_per_pixels*100)/100, mesh.dims[1] * self.inch_per_pixels)
        ...
        self.mesh = mesh
        self.image_dims = self.mesh.dims
        self.limits = self.mesh.bounds
        ...
        self.frm_bounds_x_info_value = tk.Label(master = self.frm_bounds_info, text='(' + str(mesh.bounds[0]) + ';' + str(mesh.bounds[1]) + ')', anchor=tk.W)
        ...
        self.frm_bounds_y_info_value_upper = tk.Label(master = self.frm_bounds_info, text=str(mesh.bounds[3]), anchor=tk.W)
        self.frm_bounds_y_info_value_lower = tk.Label(master=self.frm_bounds_info, text=str(mesh.bounds[2]), anchor=tk.W)
        ...
        self.canvas.get_tk_widget().bind("<Button-1>", self.image_press)
        self.canvas.get_tk_widget().bind("<ButtonRelease-1>", self.image_unpress)
    ...
    def image_press(self, event):
        self.pressed = True
        self.pressed_cords = [event.x, event.y]

    def image_unpress(self, event):
        if self.pressed:
            cord_pkg = [self.pressed_cords, [event.x, event.y]]
            new_mesh = self.mesh.get_updated_mesh_px(cord_pkg)
            self.mesh = new_mesh
            self.calculate_draw()
            self.pressed = False
    ...
    def calculate_draw(self):
        ...
        :return: None
        """
        results = fractal.get_fractal_data(self.mesh, iters=100)
...
def launch_program():
    ...
    mesh = util.Mesh(bounds, dims)
    gui = ProgramGui(mesh, device_info={NAME:info[util.NAME],
